...
| NASDAQ 100 | ^NDX | 96??8 depending on method (recent IPOs GEV/PLTR/UBER/ALAB/ABNB/CRWV/NBIS/SNDK dropped for lack of ??60 trading days at t0) |
...
| MACE-AC              | 5 bps spread (~0.05 % each side) + non-linear Almgren-Chriss impact | | | α=0.5, β=1.0, ?½=5d |
...
| Dow 30     | Dynamic Stock Recommendation  | 1,117,793 |     +11.78  |       ??.11  |     +5.12 | +15.67    |   0.40  |  0.52   |  -15.98  |  0.32  |
...
| S&P 100    | DRL Ensemble                  | 1,258,247 |     +25.82  |      ??3.17  |    +10.85 | +15.10    |   0.76  |  1.01   |  -17.13  |  0.63  |
| S&P 100    | Dynamic Stock Recommendation  | 1,644,501 |     +64.45  |      +25.46  |    +24.99 | +35.18    |   0.81  |  1.15   |  -31.10  |  0.80  |
| S&P 100    | MACE                          | 1,342,488 |     +34.25  |       ??.74  |    +14.12 | +15.63    |   0.92  |  1.27   |  -18.83  |  0.75  |
...
- **Ann Vol %** = daily-return std ? ??52
- **Sharpe** = mean(daily return) / std(daily return) ? ??52  (rf = 0)
- **Sortino** = mean(daily return) / std(negative daily return) ? ??52
...
gap in particular is dramatic ??DESQ's Sharpe (2.9??.8) is 2.3??.6? the
best peer.

### 4.2 Drawdown control is DESQ's most consistent edge

While peer methods and indices all endure double-digit drawdowns (??6 %
to ??1 %) during the 2025-04 correction and again in early 2026, DESQ
keeps drawdowns to **??.5 %, ??.9 %, ??.0 %**. This is a direct benefit
...
- **MACE** (best AC agent) ??good on Dow 30 (+66 %) where the A2C agent's
  ...
  cost model shrinks per-quarter trade counts by 5???38? vs baseline
  without materially degrading returns ??validating the paper's central
  claim.
- **DRL Ensemble** ??most balanced across universes (25??2 %) but
...
DESQ's 0.44 % roundtrip fee subtracts roughly **5?? pp of total return
...
|---|---:|---:|---:|
| Dow 30     | +74.0 %  | +68.6 %  | ??.4 pp |
| S&P 100    | +95.1 %  | +89.5 %  | ??.6 pp |
| NASDAQ 100 | +97.7 %  | +92.1 %  | ??.6 pp |
...
- Dow 30 : held +25 %  when Dow-30 index went to ?? % and DRL Ensemble to ??1 %
- S&P 100: held +35 %  when ^OEX went to ?? % and MACE to ?? %
...
1. **Frozen SB3 agents** ??Yang 2020 and MACE are handicapped vs the
   ...
   narrow the gap by 5??5 pp on those two methods but is out of scope
   for this study.
...
5. **Broker fee model** ??sell-side 0.34 % is deliberately conservative
   ...
   would likely see DESQ outperform by 3?? pp more than reported here.
