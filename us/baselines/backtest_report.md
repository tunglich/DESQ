# Backtest Report — DTE vs Three Literature Baselines
...
We benchmark our **DTE** (*Dynamic-flooding Transformer Ensembles*)
...
**Headline result** — DTE dominates every literature baseline on every
...
| Universe | DTE Total Ret | Best Peer | Excess vs Index |
|---|---:|---:|---:|
...
|---|---:|---:|---:|---|
| **DTE (ours)**       | 0.10 % | 0.34 % | **0.44 %** | US retail broker + FINRA/SEC sell fee proxy |
...
DTE therefore carries the **most conservative** cost assumption; the three
peer methods share a lower 0.10 % roundtrip so their comparison is
like-for-like.

### 2.4 Method definitions

- **DTE** — for each ticker, a frozen Dynamic-flooding transformer produces a
...
|------------|-------------------------------|----------:|------------:|-------------:|----------:|----------:|-------:|--------:|---------:|-------:|
| **Dow 30** | **DTE**                       | 1,686,224 |  **+68.62** |   **+48.73** | **+26.40**| +8.18     | **2.91**| **4.23**| **-7.87**| **3.36** |
...
| **S&P 100**| **DTE**                       | 1,895,017 |  **+89.50** |   **+50.51** | **+33.19**| **+7.69** | **3.77**| **6.06**| **-6.47**| **5.13** |
...
| **NDX 100**| **DTE**                       | 1,920,563 |  **+92.06** |   **+53.31** | **+34.00**| **+9.55** | **3.12**| **5.05**| **-8.03**| **4.23** |
...
### 4.1 DTE dominates on every axis, every universe

DTE wins **every one of the 15 relevant metric × universe cells** where it
...
gap in particular is dramatic — DTE's Sharpe (2.9–3.8) is 2.3–4.6× the
best peer.

### 4.2 Drawdown control is DTE's most consistent edge

While peer methods and indices all endure double-digit drawdowns (−16 %
to −31 %) during the 2025-04 correction and again in early 2026, DTE
...
DTE's 0.44 % roundtrip fee subtracts roughly **5–6 pp of total return
across the 27 months** compared to a hypothetical zero-cost run:

| Universe | Zero-cost DTE (earlier measurement) | Current 0.44 %-cost DTE | Cost drag |
|---|---:|---:|---:|
| Dow 30     | +74.0 %  | +68.6 %  | −5.4 pp |
| S&P 100    | +95.1 %  | +89.5 %  | −5.6 pp |
| NASDAQ 100 | +97.7 %  | +92.1 %  | −5.6 pp |

DTE would need to pay ~2.3 % roundtrip fees (5× current) before it
...
MACE (best AC) briefly overtook DTE on Dow 30 in 2026-01 (+87 % vs +82 %)
but reverted on the March correction — DTE's lower path-volatility is
...
Conda environments:
- `finlabUS` — DTE portfolio, Yang 2018 DSR, aggregator, combined chart
...
5. **Broker fee model** — sell-side 0.34 % is deliberately conservative
   ...
   would likely see DTE outperform by 3–5 pp more than reported here.
