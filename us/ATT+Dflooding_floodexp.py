...
def plot_history(history, out_path, flood_mode='dynamic', static_b=0.2, title=None):
    ...
    # --- (2) Loss + flooding b ---
    if 'ce' in history:
        ax[1].plot(epochs, history['ce'], color='#4C72B0', linestyle='-', label='Training loss (raw CE)')
    if 'loss' in history:
        ax[1].plot(epochs, history['loss'], color='#4C72B0', linestyle=':', alpha=0.8,
                   label='Training loss (flooded)')
    if 'val_loss' in history:
        ax[1].plot(epochs, history['val_loss'], color='#DD8452', linestyle='-', label='Validation loss')

    # flooding b 水位（與 loss 同 y 軸/同單位）
    b_color = '#C44E52'
    if flood_mode == 'static':
        ax[1].axhline(static_b, color=b_color, linestyle='--', linewidth=1.3,
                      label=f'Flooding b = {static_b:g}')
    elif flood_mode == 'dynamic' and 'flooding_b' in history:
        ax[1].plot(epochs, history['flooding_b'], color=b_color, linestyle='--', linewidth=1.3,
                   drawstyle='steps-post', label='Flooding b (dynamic)')
    elif flood_mode == 'none':
        ax[1].axhline(0.0, color=b_color, linestyle='--', linewidth=1.0, alpha=0.6,
                      label='Flooding b = 0')

    ax[1].set_title('Loss & flooding level b')
    ax[1].set_ylabel('Loss / b')
...
def prepare_dataset(m, n):
    ...
    n_timesteps = 1
    # 對齊 val window：可用 FORCE_TRAIN_START 環境變數覆寫 train_start，例如
    # FORCE_TRAIN_START=2007-07-10 會把所有 aspect 都對齊到同一個資料起點，使
    # walk-forward val window 落在相同日期區間。
    force_start = os.getenv('FORCE_TRAIN_START', '').strip()
    if force_start:
        # 只有在強制起點比自動偵測還晚時才採用（避免要求 fundamental 從 macro
        # 起點前更早，導致 lookback 不足）。
        try:
            if pd.Timestamp(force_start) >= pd.Timestamp(train_start):
                print(f'[ALIGN] train_start: auto={train_start} -> FORCE={force_start}')
                train_start = force_start
            else:
                print(f'[ALIGN][WARN] FORCE_TRAIN_START={force_start} earlier than auto={train_start}; ignored')
        except Exception as e:  # noqa: BLE001
            print(f'[ALIGN][WARN] bad FORCE_TRAIN_START={force_start!r}: {e}')
...
symbols = [x.strip() for x in os.getenv('STOCK_IDS', '8299').split(',') if x.strip()]
debug = True
...
model_types_list = [x.strip() for x in os.getenv('MODEL_TYPES', 'fundamental,trade,moment,sentiment,tech_trend,macro').split(',') if x.strip()]
    ...
    for train_model in model_types_list:  # 'tech_trend','moment','sentiment', 'trade' --- IGNORE ---
        ...
        if best_hp_values is not None and not os.path.exists(f'experiments_test/{train_model}{"_test" if test_mode else ""}'):
            ...
            for r in range(repeats):
                ...
                max_epochs = MAX_EPOCHS_ENV if MAX_EPOCHS_ENV > 0 else 120
                ...
                plot_history(
                    ...
                    title=f'{MODE_LABELS.get(FLOOD_MODE, FLOOD_MODE)} | ATT {train_model} {symbol} (run {r+1})'
                )
