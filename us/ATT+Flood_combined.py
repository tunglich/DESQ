...
# Combined-5 production defaults (1.5× baseline since combined features are larger):
#   stage1: 12 trials / 80 epochs  ->  18 trials / 120 epochs
#   stage2: 24 trials / 120 epochs ->  36 trials / 180 epochs
STAGE1_MAX_TRIALS = int(os.getenv('STAGE1_MAX_TRIALS', '18'))
STAGE2_MAX_TRIALS = int(os.getenv('STAGE2_MAX_TRIALS', '36'))
STAGE1_EPOCHS = int(os.getenv('STAGE1_EPOCHS', '120'))
STAGE2_EPOCHS = int(os.getenv('STAGE2_EPOCHS', '180'))

# Date range env vars (combined-5 v2 production split):
#   train+val end: 2023-12-31, test: 2024-01-01 ~ 2026-06-03 (≈14.5 yr train, 2.5 yr test)
...
# 主訓練入口：本檔為 combined-5-aspect 變體，固定使用單一 `combined5` aspect
# （由 combined_features.load_and_combine_features 將 5 個面向 inner-join concat 後輸入）
from combined_features import COMBINED_ASPECT, load_and_combine_features

stock_ids = [x.strip() for x in os.getenv('STOCK_IDS', '2330').split(',') if x.strip()]
model_types = [COMBINED_ASPECT]  # 固定單一面向；忽略 MODEL_TYPES env 以避免誤用
print(f"[COMBINED] stock_ids={stock_ids} aspect={COMBINED_ASPECT}")
...
for stock_id in stock_ids: # symbols.index.tolist():,,'2317','2308'
    for model_type in model_types:  #,'tech_trend' ,'macro' 'fundamental','tech_trend','moment','sentiment'
        try:
            ...
            # ===================== 1) 讀取資料（combined 5 aspects）=====================
            print(f"[TRAIN] {stock_id} {model_type}")
            X_y_all, train_start = load_and_combine_features(stock_id)

            n_timesteps = max(LOOKBACK_WINDOW_CHOICES)
            # Combined-5 v2 split：train+val \u7d50\u675f\u65bc TRAIN_END_DATE\uff0c\u4e26\u7559\u51fa\u8f03\u9577\u7684 test \u7a97
            val_end = TRAIN_END_DATE
            train_end = TRAIN_END_DATE
            test_start = TEST_START_DATE
            # \u82e5\u8a2d\u7684 TEST_END_DATE \u9577\u65bc\u8cc7\u6599\u6700\u5f8c\u4e00\u65e5\uff0c\u4ee5\u8cc7\u6599\u70ba\u6e96\uff1b\u53cd\u4e4b\u63a1\u7528 env \u8a2d\u5b9a
            ...
            if not DO_FEATURE_PREPROCESS:
                ...
                if model_type in ('sentiment', 'macro', 'fundamental', 'tech_trend', 'moment', 'combined5'):
