...
The `monitoring` package begins the executable implementation of Appendix F.
...
The current candidate planner is deliberately `dry_run=true` and
...
- `monitoring/config/policy_v1.json` contains thresholds that Appendix F does
  not specify numerically. These values are operational bootstrap policy, not
  paper claims.
